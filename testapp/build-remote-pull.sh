#!/usr/bin/env bash
# Focused GET-only pull gate: fake transport, real Reader and ProgressStore.
set -euo pipefail
...
MSYS_NO_PATHCONV=1 docker run --rm -v "$REPO_WIN:/io" -w /io "$IMG" bash -c '
  ...
  arm-linux-musleabi-gcc $CFLAGS -Werror -c /io/src/sync/kosync_pull.c -o kosync_pull.o
  arm-linux-musleabi-gcc $CFLAGS -Werror -c /io/src/sync/sync_pull_controller.c -o sync_pull_controller.o
  arm-linux-musleabi-gcc $CFLAGS -Werror -c /io/src/app/remote-pull-test.c -o remote-pull-test.o
  if ! arm-linux-musleabi-nm -u kosync_pull.o | grep -q " cn_kosync_get_progress$"; then
    echo "FAIL: pull does not reference authenticated GET"
    exit 1
  fi
  if arm-linux-musleabi-nm -u kosync_pull.o | grep -q "cn_kosync_put_progress"; then
    echo "FAIL: GET-only pull object references PUT"
    exit 1
  fi
  ...
  arm-linux-musleabi-g++ -static -no-pie -fno-pie -march=armv5te -O2 \
    kosync_pull.o sync_pull_controller.o remote-pull-test.o reader_sync.o \
    ui.o canvas.o text.o library.o book_identity.o progress_store.o \
    sync_controller.o kosync_sync.o kosync_policy.o kosync.o settings_store.o \
    credential_store.o storage_layout.o storage_verify.o netsimple.o \
    tlssimple.o dnssimple.o timesimple.o koreader_identity.o md5.o reader.o \
    ...
    -o /io/testapp/crossnook-remote-pull-test
  file /io/testapp/crossnook-remote-pull-test | \
    grep -q "ELF 32-bit LSB executable, ARM.*statically linked"
  readelf -h /io/testapp/crossnook-remote-pull-test | grep -q "Type:.*EXEC"
  readelf -h /io/testapp/crossnook-remote-pull-test | grep -q "Machine:.*ARM"
  readelf -h /io/testapp/crossnook-remote-pull-test | \
    grep -q "Flags:.*Version5 EABI.*soft-float ABI"
  readelf -A /io/testapp/crossnook-remote-pull-test | grep -q "Tag_CPU_arch: v5TE"
  if readelf -l /io/testapp/crossnook-remote-pull-test | grep -E "INTERP|DYNAMIC"; then
    echo "FAIL: dynamic sections present"
    exit 1
  fi
  echo "ARM static EABI5 soft-float non-PIE -> OK"
  ROOT=$(mktemp -d /tmp/crossnook-remote-pull-host.XXXXXX)
  trap "rm -rf \"\$ROOT\"" EXIT
  mkdir "$ROOT/state"
  set +e
  qemu-arm -L /opt/tc/arm-linux-musleabi-cross/arm-linux-musleabi \
    /io/testapp/crossnook-remote-pull-test --smoke \
    /io/testapp/test-font.ttf /io/testapp/reader-fixtures/valid.epub \
    "$ROOT/state" > "$ROOT/output" 2>&1
  status=$?
  set -e
  if grep -Fq -e "crossnook-synthetic-pull-account" \
     -e "crossnook-synthetic-pull-key" \
     -e "dfb450efddbb5387197c84460623675b" \
     -e "x-auth-key:" "$ROOT/output" \
     /io/docs/milestone-explicit-remote-pull.md; then
    echo "FAIL: credential in output or documentation"
    exit 1
  fi
  if [ "$status" -ne 0 ]; then
    grep -E "^\\[(OK|FAIL)\\]|^REMOTE PULL SMOKE" "$ROOT/output" || true
    echo "FAIL: remote pull smoke"
    exit 1
  fi
  ...
  if qemu-arm -L /opt/tc/arm-linux-musleabi-cross/arm-linux-musleabi \
       /io/testapp/crossnook-remote-pull-test --physical pull present \
       /tmp/crossnook-pull-absent /tmp/crossnook-pull-absent/crossnook \
  ...
  grep -E "^\\[(OK|FAIL)\\]|^REMOTE PULL SMOKE" "$ROOT/output"
  echo "REMOTE PULL HOST VALIDATION OK"
'
sha256sum "$REPO/testapp/crossnook-remote-pull-test"

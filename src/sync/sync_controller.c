...
void cn_sync_controller_result_init_internal(cn_sync_controller_result *result)
{
    if (!result) return;
...
void cn_sync_current_book_resolved(const cn_sync_resolved_config *config,
                                   cn_sync_controller_result *output)
{
    ...
    if (!config || !config->progress_store || !config->document_path ||
        !config->document_path[0] ||
        !cn_sync_valid_device_id_internal(config->device_id) ||
    ...
    sync_config.device = config->device_name;
    ...
    const cn_sync_controller_config *config)
{
    ...
    cn_sync_controller_result_init_internal(&output);
    ...
    if (cn_settings_validate(&settings) != CN_SETTINGS_OK) {
        ...
        cn_sync_current_book_resolved(&resolved, &output);
    }
    cn_credentials_clear(&credentials);
    return output;
}
